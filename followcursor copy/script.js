document.addEventListener('DOMContentLoaded', function() {
    ...
    // Design terms for the designer portfolio
    const designTerms = [
        'typography', 'layout', 'composition', 'contrast', 'harmony', 
        'balance', 'rhythm', 'scale', 'proximity', 'alignment',
        'color theory', 'whitespace', 'grid system', 'gestalt', 'hierarchy',
        'visual weight', 'tone', 'texture', 'pattern', 'perspective'
    ];
    
    const designTools = [
        'Figma', 'Sketch', 'Adobe XD', 'Illustrator', 'Photoshop',
        'InDesign', 'After Effects', 'Blender', 'Cinema 4D', 'Procreate',
        'Dimension', 'Substance', 'Premiere', 'Lightroom', 'Framer'
    ];
    
    // Theme toggle handler
    themeToggle.addEventListener('change', function() {
        document.documentElement.classList.toggle('light-theme');
        playSound(beepSound, 0.2);
        
        // Create ripple at toggle position
        const toggleRect = themeToggle.getBoundingClientRect();
        createRipple(toggleRect.left + 20, toggleRect.top + 10);
    });
    ...
    // Queue random design tips
    function queueRandomDesignTips() {
        const minDelay = 20000; // 20 seconds minimum between tips
        const maxDelay = 40000; // 40 seconds maximum
        
        const nextTipDelay = minDelay + Math.random() * (maxDelay - minDelay);
        
        setTimeout(() => {
            createDesignTip();
            queueRandomDesignTips();
        }, nextTipDelay);
    }
    ...
    // Create design tip popup
    function createDesignTip() {
        // Get random design term and tool
        const term = designTerms[Math.floor(Math.random() * designTerms.length)];
        const tool = designTools[Math.floor(Math.random() * designTools.length)];
        
        // Create tip messages
        const tipMessages = [
            `Consider ${term} when creating balanced layouts.`,
            `${term.charAt(0).toUpperCase() + term.slice(1)} is essential for effective visual hierarchy.`,
            `Master ${tool} to enhance your ${term} skills.`,
            `The best designers understand how ${term} influences user perception.`,
            `Try using ${tool} for your next ${term} experiment.`,
            `When working with ${term}, remember that less is often more.`,
            `${term.charAt(0).toUpperCase() + term.slice(1)} and ${designTerms[Math.floor(Math.random() * designTerms.length)]} work together to create visual harmony.`
        ];
        
        // Pick random tip
        const tipMessage = tipMessages[Math.floor(Math.random() * tipMessages.length)];
        
        // Create and position the tip popup (similar to error popup but styled differently)
        const tipPopup = document.createElement('div');
        tipPopup.className = 'error-popup designer-tip';
        ...
        tipPopup.style.left = popupLeft + 'px';
        tipPopup.style.top = popupTop + 'px';
        
        // Create tip content
        tipPopup.innerHTML = `
            <div class="error-header">
                <span>DESIGN TIP</span>
                <span class="error-code">TIP_${getRandomHex(4)}</span>
            </div>
            <div class="error-content typing-effect">
                ${tipMessage}
            </div>
        ...
        errorPopups.push(tipPopup); // Reuse the same array for tracking
        ...
        // Animate progress bar (slower for tips to give time to read)
        const progressFill = tipPopup.querySelector('.progress-fill');
        let progress = 0;
        const progressInterval = setInterval(() => {
            progress += 0.7; // Slower progress
            if (progress >= 100) {
                progress = 100;
                clearInterval(progressInterval);
                
                // Remove tip popup after completion
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        if (tipPopup.parentNode) {
                            tipPopup.parentNode.removeChild(tipPopup);
                            const index = errorPopups.indexOf(tipPopup);
                            if (index > -1) {
                                errorPopups.splice(index, 1);
                            }
                        }
